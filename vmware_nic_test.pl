...
foreach my $host (@$host_views) {
  ...
  ########## Print information on the VMs and the Hosts
    foreach my $vm (@$vm_views) {
        ...
        #if (($vm->name) eq 'dstsmesbapp03'){
            #push (@bad_servers, $vm->name);
            print "==============================================================";
            print "\nName--->", $vm->name;
            #print "\nMAC Address--->", $vm->config->hardware->device->VirtualVmxnet3->macAddress;
            #print "\nadapterType--->", $vm->config->hardware->addressType;
            print "\n test --->", $vm->config->hardware->device;
            @test = $vm->config->hardware->device;
            print @test[0]; 
            foreach $x (@test){
                print 'hi';
