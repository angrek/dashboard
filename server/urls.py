...
urlpatterns = patterns('',
    ...
    url(r'^stacked_column/(?P<service>(.*)+)/(?P<period>(.*)+)/(?P<time_range>(.*)+)$', views.stacked_column, name='stacked_column'),
    url(r'^line_basic/(?P<string>(.*)+)/(?P<string2>(.*)+)$', views.line_basic, name='line_basic'),
