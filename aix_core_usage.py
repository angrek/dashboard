...
from server.models import Frame, Power7Inventory
import logging
...
def populate():
    ...
    for frame in frames[:4]:
        ...
        for type in types:
            ...
            command = 'lshwres -r ' + type + ' -m ' + frame.rstrip() + ' --level sys -F installed_sys_proc_units:configurable_sys_proc_units'
            print command
            ...
            for line in pool_data:
                line = line.rstrip()
                print line
                if line == "TThe managed system does not support multiple shared processor pools.":
                    x = line.split(',')
