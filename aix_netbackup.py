    ...
    for server in server_list:
        ...
        if AIXServer.objects.filter(name=server):
            ...
            if test_server.ping(server):
                ...
                #FIXME what is the below line doing here??? nothing???
                ...
                if str(netbackup_version) != str(server.netbackup):
                    AIXServer.objects.filter(name=server, exception=False, active=True).update(netbackup=netbackup_version, modified=timezone.now())
